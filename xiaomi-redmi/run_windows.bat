@echo off
chcp 65001 >nul
setlocal EnableDelayedExpansion
title Xiaomi / Redmi Notification Fix Tool (Windows) - Auto Scan
color 0e

echo ======================================================================
echo    XIAOMI / REDMI NOTIFICATION FIX TOOL (WINDOWS)
echo       Fix tre thong bao cho HyperOS ^& MIUI noi dia Trung
echo       Tinh nang: Quet tu dong app tren may + Chon app bang so
echo       Tac gia: Vu Do (vudovn) - github.com/vudovn
echo ======================================================================
echo.

:: 1. KIỂM TRA VÀ TỰ TẢI ADB
set "BIN_DIR=%~dp0..\bin"
set "ADB_BIN=adb"

where adb >nul 2>nul
if %errorlevel% equ 0 (
    echo [✔] Da tim thay ADB tren he thong!
    goto :CHECK_DEVICE
)

if exist "%BIN_DIR%\platform-tools\adb.exe" (
    set "ADB_BIN=%BIN_DIR%\platform-tools\adb.exe"
    echo [✔] Da tim thay ADB cuc bo!
    goto :CHECK_DEVICE
)

echo [!] Chua co ADB. Dang tu dong tai Google Platform-Tools ve...
if not exist "%BIN_DIR%" mkdir "%BIN_DIR%"
set "URL=https://dl.google.com/android/repository/platform-tools-latest-windows.zip"
set "ZIP_PATH=%BIN_DIR%\platform-tools.zip"
echo [*] Dang tai tu: %URL%
powershell -Command "[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; (New-Object System.Net.WebClient).DownloadFile('%URL%', '%ZIP_PATH%')"
echo [*] Dang giai nen ADB...
powershell -Command "Expand-Archive -Path '%ZIP_PATH%' -DestinationPath '%BIN_DIR%' -Force"
del /f /q "%ZIP_PATH%" >nul 2>nul
set "ADB_BIN=%BIN_DIR%\platform-tools\adb.exe"
echo [✔] Da thiet lap xong ADB!
echo.

:: 2. KIỂM TRA THIẾT BỊ
:CHECK_DEVICE
echo [*] Dang kiem tra ket noi dien thoai Xiaomi qua USB...
"%ADB_BIN%" start-server >nul 2>nul

for /f "tokens=1,2" %%A in ('"%ADB_BIN%" devices ^| findstr /v "List" ^| findstr /r "[a-zA-Z0-9]"') do (
    if "%%B"=="device" goto :DEVICE_OK
    if "%%B"=="unauthorized" (
        echo [!] Thiet bi chua duoc uy quyen! Mo khoa man hinh va bam Cho phep.
        pause
        goto :CHECK_DEVICE
    )
)
echo [X] Chua phat hien dien thoai nao!
echo   1. Vao Cai dat ^> Gioi thieu dien thoai ^> Bam 7 lan vao 'Phien ban OS / MIUI'.
echo   2. Vao Tuy chon nha phat trien ^> Bat 'Go loi USB'.
echo   3. Cam cap, chon 'Luon cho phep tu may tinh nay' roi nhan OK.
pause
goto :CHECK_DEVICE

:DEVICE_OK
for /f "delims=" %%I in ('"%ADB_BIN%" shell getprop ro.product.model') do set "DEV_MODEL=%%I"
for /f "delims=" %%I in ('"%ADB_BIN%" shell getprop ro.build.display.id') do set "DEV_ROM=%%I"
echo [✔] Da ket noi: %DEV_MODEL% (ROM: %DEV_ROM%)
echo.

:: 3. MENU CHÍNH
:MENU
cls
echo ======================================================================
echo    XIAOMI / REDMI NOTIFICATION FIX TOOL (WINDOWS)
echo    Thiet bi: %DEV_MODEL% ^| ROM: %DEV_ROM%
echo ======================================================================
echo.
echo VUI LONG CHON CHUC NANG:
echo   [1] 🔔 Fix thong bao (Quet app → Chon app can whitelist)
echo   [2] 🔍 Kiem tra trang thai thiet bi ^& ket noi Google Push
echo   [3] 🔄 Khoi phuc he thong ve mac dinh
echo   [0] ❌ Thoat
echo ======================================================================
set /p choice="Nhap lua chon [0-3]: "

if "%choice%"=="1" goto :FIX_NOTI
if "%choice%"=="2" goto :STATUS
if "%choice%"=="3" goto :RESTORE
if "%choice%"=="0" goto :EXIT
goto :MENU

:: =====================================================================
:: FIX THÔNG BÁO 6 BƯỚC - QUÉT TỰ ĐỘNG
:: =====================================================================
:FIX_NOTI
echo.
echo ======================================================================
echo       FIX TRE THONG BAO XIAOMI (HYPEROS / MIUI) - QUET TU DONG
echo ======================================================================

echo [*] [1/6] Noi long Phantom Process Killer...
"%ADB_BIN%" shell /system/bin/device_config put activity_manager max_phantom_processes 2147483647 >nul 2>nul
"%ADB_BIN%" shell device_config put activity_manager max_cached_processes 256 >nul 2>nul
"%ADB_BIN%" shell device_config put activity_manager max_empty_time_millis 43200000 >nul 2>nul
echo   [✔] max_phantom_processes = vo han

echo [*] [2/6] Tat Adaptive Battery ^& App Standby...
"%ADB_BIN%" shell settings put global adaptive_battery_management_enabled 0 >nul 2>nul
"%ADB_BIN%" shell settings put global app_standby_enabled 0 >nul 2>nul
"%ADB_BIN%" shell settings put global forced_app_standby_enabled 0 >nul 2>nul
"%ADB_BIN%" shell settings put global aggressive_battery 0 >nul 2>nul
echo   [✔] Da tat Adaptive Battery.

echo [*] [3/6] Tu dong whitelist dich vu Google ^& Xiaomi Push...
for %%P in (
    com.google.android.gms com.google.android.gsf com.android.vending
    com.xiaomi.xmsf com.xiaomi.xmsfkeeper com.miui.notification
) do (
    "%ADB_BIN%" shell dumpsys deviceidle whitelist +%%P >nul 2>nul
    echo   [✔] Auto: %%P
)

echo [*] [4/6] Quet tat ca ung dung ben thu 3 tren may...
echo [*] Dang quet...
echo.

set "APP_COUNT=0"
for /f "tokens=*" %%A in ('"%ADB_BIN%" shell pm list packages -3 -e 2^>nul') do (
    set "RAW=%%A"
    set "PKG=!RAW:package:=!"
    set "SKIP=0"
    if /i "!PKG!"=="com.google.android.gms" set "SKIP=1"
    if /i "!PKG!"=="com.google.android.gsf" set "SKIP=1"
    if /i "!PKG!"=="com.android.vending" set "SKIP=1"
    if /i "!PKG!"=="com.xiaomi.xmsf" set "SKIP=1"
    if /i "!PKG!"=="com.xiaomi.xmsfkeeper" set "SKIP=1"
    if /i "!PKG!"=="com.miui.notification" set "SKIP=1"
    if "!SKIP!"=="0" (
        set /a APP_COUNT+=1
        set "APP_!APP_COUNT!=!PKG!"
        echo   [!APP_COUNT!] !PKG!
    )
)

if %APP_COUNT% equ 0 (
    echo [!] Khong tim thay app ben thu 3 nao.
    goto :SKIP_WHITELIST
)

echo.
echo ─────────────────────────────────────────────────────
echo Tim thay %APP_COUNT% ung dung ben thu 3.
echo.
echo 📌 Nhap so: 1,3,5,7  hoac  "all" / Enter = TAT CA
echo.
set /p "NOTI_SEL=Nhap so app can fix thong bao: "

if "%NOTI_SEL%"=="" set "NOTI_SEL=all"

echo.
echo [*] Dang whitelist + Active app da chon...

if /i "%NOTI_SEL%"=="all" (
    for /L %%i in (1,1,%APP_COUNT%) do (
        "%ADB_BIN%" shell dumpsys deviceidle whitelist +!APP_%%i! >nul 2>nul
        "%ADB_BIN%" shell am set-standby-bucket !APP_%%i! active >nul 2>nul
        echo   [✔] Whitelist + Active: !APP_%%i!
    )
) else (
    for %%N in (%NOTI_SEL%) do (
        set "IDX=%%N"
        set "IDX=!IDX: =!"
        if defined APP_!IDX! (
            "%ADB_BIN%" shell dumpsys deviceidle whitelist +!APP_%%N! >nul 2>nul
            "%ADB_BIN%" shell am set-standby-bucket !APP_%%N! active >nul 2>nul
            echo   [✔] Whitelist + Active: !APP_%%N!
        )
    )
)

:SKIP_WHITELIST

echo [*] [5/6] Giu ket noi WiFi ^& FCM khi tat man hinh...
"%ADB_BIN%" shell settings put global wifi_sleep_policy 2 >nul 2>nul
"%ADB_BIN%" shell settings put global wifi_idle_ms 2147483647 >nul 2>nul
"%ADB_BIN%" shell settings put global always_finish_activities 0 >nul 2>nul
"%ADB_BIN%" shell settings put global wifi_wakeup_enabled 1 >nul 2>nul
echo   [✔] WiFi luon giu ket noi.

echo [*] [6/6] Toi uu PowerKeeper ^& JobScheduler...
"%ADB_BIN%" shell setprop persist.sys.max_bg_processes 60 >nul 2>nul
"%ADB_BIN%" shell settings put global restrict_background 0 >nul 2>nul
"%ADB_BIN%" shell device_config put jobscheduler qc_timing_constraints_enabled false >nul 2>nul
"%ADB_BIN%" shell device_config put jobscheduler qc_updated_jobs_per_uid_window 99999 >nul 2>nul
echo   [✔] Da toi uu JobScheduler.

echo.
echo [🎉] DA HOAN TAT FIX THONG BAO CHO XIAOMI!
pause
goto :MENU

:: =====================================================================
:: KIỂM TRA TRẠNG THÁI
:: =====================================================================
:STATUS
echo.
echo ======================================================================
echo            KIEM TRA TRANG THAI THIET BI XIAOMI
echo ======================================================================

echo.
echo 1. Phantom Process Killer:
for /f "delims=" %%I in ('"%ADB_BIN%" shell /system/bin/device_config get activity_manager max_phantom_processes 2^>nul') do set "PHANTOM_VAL=%%I"
if "%PHANTOM_VAL%"=="2147483647" (
    echo    [✔] DA MO GIOI HAN ^(vo han^)
) else (
    echo    [!] Dang gioi han o: %PHANTOM_VAL%
)

echo.
echo 2. Adaptive Battery:
for /f "delims=" %%I in ('"%ADB_BIN%" shell settings get global adaptive_battery_management_enabled 2^>nul') do set "ADAPTIVE_VAL=%%I"
for /f "delims=" %%I in ('"%ADB_BIN%" shell settings get global app_standby_enabled 2^>nul') do set "STANDBY_VAL=%%I"
if "%ADAPTIVE_VAL%"=="0" if "%STANDBY_VAL%"=="0" (
    echo    [✔] DA TAT
) else (
    echo    [!] DANG BAT
)

echo.
echo 3. WiFi Sleep Policy:
for /f "delims=" %%I in ('"%ADB_BIN%" shell settings get global wifi_sleep_policy 2^>nul') do set "WIFI_POLICY=%%I"
if "%WIFI_POLICY%"=="2" (
    echo    [✔] LUON GIU KET NOI
) else (
    echo    [!] Co the ngat khi tat man hinh
)

echo.
echo 4. Google Push (FCM):
for /f "delims=" %%I in ('"%ADB_BIN%" shell dumpsys activity service com.google.android.gms/.gcm.GcmService ^| findstr /i "connected="') do (
    echo    [✔] %%I
)

echo.
echo 5. Doze Whitelist (user):
for /f "delims=" %%I in ('"%ADB_BIN%" shell dumpsys deviceidle whitelist ^| findstr "user,"') do (
    echo    [✔] %%I
)
echo.
pause
goto :MENU

:: =====================================================================
:: KHÔI PHỤC
:: =====================================================================
:RESTORE
echo.
echo ======================================================================
echo            KHOI PHUC CAI DAT GOC XIAOMI
echo ======================================================================
echo [*] Khoi phuc Phantom Process Killer...
"%ADB_BIN%" shell /system/bin/device_config put activity_manager max_phantom_processes 32 >nul 2>nul
echo   [✔] max_phantom_processes = 32 (mac dinh)

echo [*] Bat lai Adaptive Battery...
"%ADB_BIN%" shell settings put global adaptive_battery_management_enabled 1 >nul 2>nul
"%ADB_BIN%" shell settings put global app_standby_enabled 1 >nul 2>nul
echo   [✔] Adaptive Battery = BAT

echo [*] WiFi Sleep Policy...
"%ADB_BIN%" shell settings put global wifi_sleep_policy 0 >nul 2>nul
echo   [✔] WiFi = mac dinh

echo [*] Xoa toan bo Doze Whitelist (user)...
for /f "delims=" %%I in ('"%ADB_BIN%" shell dumpsys deviceidle whitelist ^| findstr "user,"') do (
    set "LINE=%%I"
    for /f "tokens=2 delims=," %%P in ("!LINE!") do (
        "%ADB_BIN%" shell dumpsys deviceidle whitelist -%%P >nul 2>nul
        echo   [✔] Xoa whitelist: %%P
    )
)
echo [🎉] Da khoi phuc cai dat goc Xiaomi!
pause
goto :MENU

:EXIT
echo Tam biet! Cam on ban da su dung tool.
timeout /t 2 >nul
exit /b
